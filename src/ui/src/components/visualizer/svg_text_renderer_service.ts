...
import {DEFAULT_NODE_ATTRS_TABLE_FONT_SIZE, NODE_ANIMATION_DURATION, NODE_ATTRS_TABLE_FONT_SIZE_TO_HEIGHT_RATIO,} from './common/consts';
import {ModelNode} from './common/model_graph';
import {RenderElement, RenderElementNode, RenderElementType, WebglColor,} from './common/types';
import {getNodeLabelHeight, getNodeLabelLineHeight, isGroupNode, isOpNode,} from './common/utils';
...
export class SvgTextRendererService {
  ...
  render(zoomTargetElement: HTMLElement|SVGElement, clearAllFirst = false) {
    ...
    const elementUpdateTransition =
        elementUpdate.transition()
            .ease(d3.easeSinOut)
    ...
    nodeLabelUpdate.attr('x', (d) => this.getNodeLabelX(d))
        .attr('y', (d) => this.getNodeLabelY(d))
        .style('fill', (d) => this.getNodeLabelColor(d))
        .style('font-size', (d) => this.getNodeLabelFontSize(d));
    ...
    const scale = fontSize /
        this.webglRenderer.webglRendererAttrsTableService
            .getWebglTextsFontSize();
    ...
    for (const node of nodeElementsToRender) {
      const {rows} = this.webglRenderer.webglRendererAttrsTableService
      ...
      const nodeAttrsTable =
          g.select(`g.node[data-id="${this.normalizeId(node.id)}"]`)
              .append('g')
      ...
      if (rows.length > 0 && isOpNode(node)) {
        const padding = 16;
        const width = this.webglRenderer.getNodeWidth(node, true) -
            this.webglRendererThreejsService.convertXFromSceneToScreen(
                -padding);
        nodeAttrsTable.append('rect')
            ...
            .attr(
                ...
                this.webglRendererThreejsService.convertZFromSceneToScreen(
                    rows[rows.length - 1].keyLabelData.z -
                        rows[0].keyLabelData.z + rowHeight,
                    ),
                )
            .attr('rx', 2)
            .attr(
      ...
      for (const row of rows) {
        const textElement = nodeAttrsTable.append('text').style(
            'font-size', attrsTableFontSize);
  ...
  setTextsOpacity(
    ...
    const selectors =
        nodeIds.map((nodeId) => `g.node[data-id="${this.normalizeId(nodeId)}"]`)
            .join(', ');
  ...
  private getRgbaColor(webglColor: WebglColor|undefined): string {
    return `rgba(${(webglColor?.r ?? 0) * 255}, ${
        (webglColor?.g ?? 0) * 255}, ${(webglColor?.b ?? 0) * 255}, 1)`;
  }
