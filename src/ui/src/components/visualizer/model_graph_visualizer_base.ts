...
/** Base class for model graph visualizer component. */
@Directive()
export abstract class ModelGraphVisualizerBase
  implements OnInit, OnDestroy, OnChanges
{
  ...
  readonly nodeDataProviderExtensionService = inject(
    NodeDataProviderExtensionService,
  );
  readonly nodeStylerService = inject(NodeStylerService);
  ...
  protected findGraphFromCollections(
    collectionLabel: string,
    graphId: string,
  ): Graph | undefined {
  ...
  protected handleGetNodeDataPathFromPostMessage(path: string) {
  ...
  protected handleSelectNodeByOutputTensorNameFromPostMessage(
    tensorName: string,
  ) {
  ...
  protected handleSelectNodeByNodeIdFromPostMessage(nodeId: string) {
    const modelGraph = this.appService.getModelGraphFromSelectedPane();
    if (!modelGraph) {
      return;
    }
