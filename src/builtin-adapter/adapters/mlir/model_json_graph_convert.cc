...
#include "adapters/mlir/transforms/conversion.h"
...
absl::StatusOr<std::string> ModuleOpToJson(const VisualizeConfig& config,
  ...
  ABSL_ASSIGN_OR_RETURN(Graph graph, MlirToGraph(config, module_op));
...
absl::StatusOr<std::string> ConvertSavedModelToJson(
  ...
  ABSL_ASSIGN_OR_RETURN(TfMetadata tf_metadata, ObtainTfMetadata(model_path));
  ...
  if (tf_metadata.tf_version == 1) {
    ABSL_LOG(INFO) << "Converting SavedModel V1 to MLIR module...";
    ...
    ABSL_RETURN_IF_ERROR(RunTFShapeInference(*module_op, diag_handler));
  } else {
    ABSL_LOG(INFO) << "Converting SavedModel V2 to MLIR module...";
    // Converts SavedModel V2 to MLIR module.
    ABSL_ASSIGN_OR_RETURN(module_op,
    ...
    ABSL_RETURN_IF_ERROR(RunStandardPipeline(*module_op, diag_handler));
  }
  ...
  if (HasXlaCallModule(*module_op)) {
    ...
    // proceeding.
    ABSL_RETURN_IF_ERROR(ConvertToStablehloModule(*module_op, diag_handler));
  }
...
absl::StatusOr<std::string> ConvertFlatbufferToJson(
  ...
  if (is_modelpath) {
    ABSL_RETURN_IF_ERROR(tsl::ReadFileToString(
        tsl::Env::Default(), std::string(model_path_or_buffer),
        &model_content));
...
absl::StatusOr<std::string> ConvertMlirToJson(const VisualizeConfig& config,
  ...
  registry.insert<mlir::TFL::TensorFlowLiteDialect, mlir::TF::TensorFlowDialect,
                  ...
                  mlir::sdy::SdyDialect, mlir::tosa::TosaDialect>();
  ...
  ABSL_RETURN_IF_ERROR(DeserializeVhloToStablehlo(*module_op, diag_handler));
