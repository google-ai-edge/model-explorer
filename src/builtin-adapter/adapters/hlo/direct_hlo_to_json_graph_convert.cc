...
#include "adapters/hlo/direct_hlo_to_json_graph_convert.h"
...
absl::Status BuildHloInstructionNode(
  ...
  // Add incoming edges.
  ABSL_RETURN_IF_ERROR(AddHloInstructionIncomingEdges(
...
absl::Status HloComputationToGraphImpl(
  ...
  if (computation.FusionInstruction() != nullptr) {
    ABSL_RETURN_IF_ERROR(BuildHloInstructionNode(
  ...
  for (const xla::HloInstruction* instruction :
    ...
    if (instruction->opcode() == xla::HloOpcode::kFusion &&
      ...
      ABSL_RETURN_IF_ERROR(HloComputationToGraphImpl(
...
absl::StatusOr<std::string> HloGraphAdapter(
  ...
  ABSL_ASSIGN_OR_RETURN(GraphCollection graph_collection,
