...
absl::Status ReadGraphDef(absl::string_view model_path,
  ...
  if (!ReadBinaryProto(tsl::Env::Default(), model_path, &graph_def).ok()) {
    return ReadTextProto(tsl::Env::Default(), model_path, &graph_def);
  }
...
absl::Status AddTensorInfo(absl::string_view attr_name,
  ...
  ABSL_ASSIGN_OR_RETURN(const std::string tensor_string,
...
absl::Status AddListOfTensorInfo(
  ...
  for (int i = 0; i < tensors.size(); ++i) {
    ...
    ABSL_RETURN_IF_ERROR(AddTensorInfo(attr_name, tensor, config, i, builder));
  }
  return absl::OkStatus();
}
...
absl::Status AddFunctionAttribute(
  ...
  ABSL_ASSIGN_OR_RETURN(const std::string function_name,
...
absl::Status AddListAttribute(
  ...
  if (!list.s().empty()) {
    ...
    ABSL_RETURN_IF_ERROR(ProcessListOfFunctionsAttribute(
        attr_name, functions_set, list.func(), builder));
  }
  return absl::OkStatus();
}
...
absl::Status AddAttributeInformation(
  ...
  for (auto it = node_def.attr().begin(); it != node_def.attr().end(); ++it) {
    ...
    if (attr_value.has_s()) {
      ...
      ABSL_RETURN_IF_ERROR(AddTensorInfo(attr_name, attr_value.tensor(), config,
                                         /*relative_idx=*/0, builder));
    } else if (attr_value.has_list()) {
      ABSL_RETURN_IF_ERROR(AddListAttribute(attr_name, functions_set, config,
                                            attr_value.list(), builder));
    } else if (attr_value.has_func()) {
      ABSL_RETURN_IF_ERROR(AddFunctionAttribute(attr_name, attr_value.func(),
...
absl::Status AddSubgraph(
  ...
  for (const tensorflow::NodeDef& node_def : node_defs) {
    ABSL_ASSIGN_OR_RETURN(const std::string immediate_name,
    ...
    builder.SetNodeInfo(node_to_id[node_def.name()], node_label,
                        node_def.name());
    ABSL_RETURN_IF_ERROR(AddAttributeInformation(
        node_def, config, immediate_name, functions_set, builder));
  ...
  if (func_def.has_value()) {
    ABSL_RETURN_IF_ERROR(AddGraphOutputsNode(
        *func_def, input_to_idx, node_to_id, &next_node_id, subgraph));
  }
...
absl::Status AddSubgraphsForFunctions(
  ...
  for (const tensorflow::FunctionDef& function_def : library.function()) {
    ABSL_RETURN_IF_ERROR(AddSubgraph(function_def.signature().name(), config,
...
absl::Status AddAllSubgraphsFromGraphDef(const VisualizeConfig& config,
                                         ...
                                         Graph& graph) {
  ABSL_ASSIGN_OR_RETURN(const absl::flat_hash_set<std::string> functions_set,
                        GetNamesOfAllFunctions(graph_def.library()));

  ABSL_RETURN_IF_ERROR(AddSubgraph("main", config, graph_def.node(),
                                   functions_set,
                                   /*func_def=*/std::nullopt, graph));

  ABSL_RETURN_IF_ERROR(AddSubgraphsForFunctions(config, graph_def.library(),
                                                functions_set, graph));
...
absl::StatusOr<std::string> ConvertGraphDefDirectlyToJsonImpl(
  ...
  Graph graph;
  ABSL_RETURN_IF_ERROR(AddAllSubgraphsFromGraphDef(config, graph_def, graph));
...
absl::StatusOr<std::string> ConvertGraphDefDirectlyToJson(
  ...
  tensorflow::GraphDef graph_def;
  ABSL_RETURN_IF_ERROR(ReadGraphDef(model_path, graph_def));
...
absl::StatusOr<std::string> ConvertSavedModelDirectlyToJson(
  ...
  ABSL_RETURN_IF_ERROR(tensorflow::ReadSavedModel(model_path, &saved_model));
